ITEM.name = "Deadlock"
ITEM.description = "A heavy physical lock that can be attached to a door and shot off."
...
if (CLIENT) then
    function ITEM:PopulateTooltip(tooltip)
        ...
        info:SetText("Attach to a door to create a physical, destructible lock.")
...
ITEM.functions.Place = {
    ...
    OnRun = function(item)
        ...
        if (not plugin or not plugin.BeginPlacement) then
            client:Notify("Door locks system is not ready.")
            return false
        end

        -- Store trace data for deadlock placement
        local traceData = {
            hitPos = trace.HitPos,
            hitNormal = trace.HitNormal
        }

        plugin:BeginPlacement(client, door, plugin.lockTypes.DEADLOCK, item.id, traceData)
        
        -- Mark that placement has started for this item
        item:SetData("placementActive", true)

        -- consumed only when placement succeeds
        return false
    end
}
    ...
    OnRun = function(item)
        ...
        elseif (SERVER) then
            ...
            if (plugin and plugin.placementSessions) then
                ...
                if (session and #session.doorIDs > 0 and session.lockType == plugin.lockTypes.DEADLOCK and session.itemID == item.id) then
